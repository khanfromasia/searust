...
impl Model for SqliteModel {
    ...
    fn add_document(&mut self, file_path: PathBuf, content: &[char]) -> Result<(), ()> {
        ...
        for term in &terms {
            let freq = {
                let query = "SELECT freq FROM term_freq WHERE doc_id = :doc_id AND term = :term";
                let log_err = |err| {
                    eprintln!("ERROR: could not prepare or execute query {query}: {err}");
                };
                let mut stmt = self.connection.prepare(query).map_err(log_err)?;
            ...
            let query = "INSERT OR REPLACE INTO term_freq(doc_id, term, freq) VALUES (:doc_id, :term, :freq)";
            let log_err = |err| {
