...
fn search_handler(mut request: Request, model: &impl Model) -> io::Result<()> {
    let mut buf = Vec::new();
    ...
    let result = match model.search_query(&body) {
        Ok(result) => result,
        Err(()) => return serve_error(request, 500, "could not find in the model"),
    };
...
fn serve_request(request: Request, model: &impl Model) -> io::Result<()> {
